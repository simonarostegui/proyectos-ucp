...
void cargarVotos(int votos[5][3]) {
    ...
    for (int i = 0; i < 5; i++) {
        for (int j = 0; j < 3; j++) {
            votos[i][j] = rand() % 301; // Generar número aleatorio entre 0 y 300
            printf("Votos generados para el partido %d en la localidad %d: %d\n", j + 1, i + 1, votos[i][j]);
        }
    }
};
...
int main() {
    ...
    while (opcion == 1) {
        printf("Desea buscar votos por localidad y partido? (1 para si, 0 para no): ");
        scanf("%d", &opcion);
        if (opcion == 1) {
            buscarVotos(votos);
        }
