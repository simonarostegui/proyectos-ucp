...
def Cambiar_Desafio_Loop(desafios_ronda,d,ron):
	...
	while True:
		...
		for des in range(len(desafios_ronda)):
			...
			if des==d:
				actual=crear_imagen(RONDAS_DIR,desafios_ronda[des]["Seleccion_icon_lit"],x+250,200,True,False)
			elif des>d:
				crear_imagen(RONDAS_DIR,desafios_ronda[des]["Seleccion_icon"],x+250,200,False,False)
			else:
				crear_imagen(RONDAS_DIR,desafios_ronda[des]["Seleccion_icon_down"],x+250,200,False,False)
			x+=250
		dibujarTexto("RONDA "+str(ron+1),pygame.font.Font('TrajanPro-Regular.otf', 40),window,537,30,NEGRO)
		boton_comenzar=Boton("comenzar.png","comenzar_lit.png",RONDAS_DIR,640,600)
